...
pub fn run(ctx: &Context) -> Result<()> {
  ...
  for (category, files) in common_files {
    for file in *files {
      let path = ctx.home_path.join(file);
  ...
  for (category, dirs_list) in common_dirs {
    for dir in *dirs_list {
      let logical_dir = std::path::Path::new(dir);
      let path = ctx.home_path.join(dir);
      if path.exists() && !managed.iter().any(|entry| entry.starts_with(logical_dir)) {
        suggestions
          .entry(*category)
          .or_default()
          .push(dir.to_string());
